...
```bash
donut_or_not$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
donut_or_not$ sam local start-api
donut_or_not$ curl http://localhost:3000/classify_digit
```
...
```bash
donut_or_not$ sam logs -n InferenceFunction --stack-name donut_or_not --tail
```
...
```bash
aws cloudformation delete-stack --stack-name donut_or_not
```
